...
// Who can edit/delete the project itself (mirrors the backend's permit("SUPER_ADMIN","ADMIN")).
...
const ProjectBoard = () => {
  ...
  const onUpdateProject = async (e) => {
    ...
    try {
      await api.put(`/projects/${projectId}`, { ...editForm, workspaceId });
      setEditModal(false);
  ...
  const onDeleteProject = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/projects/${projectId}`, { params: { workspaceId, backup: deleteBackup } });
      navigate(`/workspaces/${workspaceId}`);
    } catch (err) {
      setDeleteError(err.response?.data?.message || "Couldn't delete project.");
      setDeleting(false);
    }
  };
    ...
    <AppShell>
      ...
      >
        <p className="mb-4 text-sm text-ink-soft">
          This permanently deletes <span className="font-medium text-ink">{project?.name}</span>, including all
          its tasks, comments, and members. This can't be undone from here.
        </p>
        ...
        <label className="mb-4 flex items-start gap-2 text-sm text-ink">
          ...
          />
          <span>
            Back up the project's data before deleting
            <span className="block text-xs text-ink-faint">
              Saves a snapshot (tasks, comments, members) that an Admin can restore later from
              this workspace's "Deleted projects" panel.
            </span>
          </span>
        </label>
